...
class TERAGGraph:
    ...
    # passage -> concepts it contains
    passage_to_concepts: Dict[str, Set[str]] = field(default_factory=lambda: defaultdict(set))
    # concept -> passages that contain it
    concept_to_passages: Dict[str, Set[str]] = field(default_factory=lambda: defaultdict(set))
    ...
    def add_edge(self, passage_id: str, concept_id: str):
        """Add bidirectional edge between passage and concept"""
        self.passage_to_concepts[passage_id].add(concept_id)
        self.concept_to_passages[concept_id].add(passage_id)
    ...
    def get_passage_neighbors(self, passage_id: str) -> Set[str]:
        """Get all concept neighbors of a passage"""
        return self.passage_to_concepts.get(passage_id, set())

    def get_concept_neighbors(self, concept_id: str) -> Set[str]:
        """Get all passage neighbors of a concept"""
        return self.concept_to_passages.get(concept_id, set())
    ...
    def to_networkx(self, include_node_attributes: bool = True, include_edge_weights: bool = False) -> 'nx.Graph':
        """
        ...
        # Add edges
        for passage_id, concept_ids in self.passage_to_concepts.items():
            for concept_id in concept_ids:
                if concept_id in self.concepts:
                    edge_attrs = {}
                    
                    if include_edge_weights:
                        # Weight based on concept frequency (inverse)
                        frequency = self.concepts[concept_id].frequency
                        weight = 1.0 / frequency if frequency > 0 else 1.0
                        edge_attrs['weight'] = weight
                        edge_attrs['concept_frequency'] = frequency
...
class GraphBuilder:
    """
    Builds TERAG graph from document chunks
    """
